---
title: Docker 入门（一）：容器化部署与基本概念
description: 从部署环境不一致的问题理解 Docker，认识镜像、容器、Dockerfile 和仓库，并运行第一个容器。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 写给懒猫微服玩家的容器小书 Docker篇（一）：《无法部署的诅咒》
---

同一份代码换到另一台机器就跑不起来，常见原因是依赖、配置和运行环境不同。这一章从部署问题讲起，介绍 Docker 的基本概念、安装和第一个容器；示例使用标准 Docker 命令。容器能打包应用与依赖，但宿主机内核和 CPU 架构仍然影响运行条件。

本文属于「容器小书 · 第一章《无法部署的诅咒》」。[查看系列目录](/series/docker/) · [Docker 跨架构运行与镜像迁移](/86ab1d7c/)。
