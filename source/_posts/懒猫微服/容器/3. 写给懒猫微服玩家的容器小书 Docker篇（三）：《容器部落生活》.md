---
title: Docker 容器管理（三）：启动、停止、日志与重启策略
description: 练习 docker run、ps、stop、start、exec 和 logs，理解前后台运行、端口映射、环境变量与容器重启策略。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 写给懒猫微服玩家的容器小书 Docker篇（三）：《容器部落生活》
---

容器启动后立刻退出、服务没有监听预期端口、重启以后应用没有恢复，这些问题都需要先看容器的状态和生命周期。本章集中讲启动、停止、交互、日志与重启策略，帮助你把一个镜像变成可观察、可管理的服务。

本文属于「容器小书 · 第三章《容器部落生活》」。[查看系列目录](/series/docker/) · [另一类自启任务：systemd 用户服务](/95e86a3c/)。
