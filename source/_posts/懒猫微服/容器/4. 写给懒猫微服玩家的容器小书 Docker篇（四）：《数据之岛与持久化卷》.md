---
title: Docker 数据持久化（四）：Volume、Bind Mount 与备份恢复
description: 对照 Docker Volume 与 Bind Mount，练习数据卷创建、容器共享、目录挂载以及备份恢复，并延伸到 Kubernetes PVC。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 写给懒猫微服玩家的容器小书 Docker篇（四）：《数据之岛与持久化卷》
---

重新创建容器时，数据库和上传文件需要留在容器之外。本章对照 Volume 和 Bind Mount，说明如何挂载、共享、备份和恢复数据，并继续看这些做法如何用于 CI/CD 与 Kubernetes。操作前先分清容器可写层、宿主目录和数据卷，避免把清理容器变成误删数据。

本文属于「容器小书 · 第四章《数据之岛与持久化卷》」。[查看系列目录](/series/docker/) · [在 Compose 中配置数据卷和多服务依赖](/61cfcd24/)。
...
有一天，小李运行了一个容器，里面的 Flask 项目能正常写入用户信息到 SQLite 数据库。可当容器被删除、重新创建后——原来保存在容器里的数据消失了！
