---
title: Debian 与 Ubuntu 配置 XRDP：XFCE 远程桌面与黑屏排查
description: 为 Debian 系系统安装 XRDP 与 XFCE，配置会话、用户登录和启动脚本，并记录懒猫微服上的远程桌面实践。
tags:
- 懒猫微服
toc: true
series: 懒猫微服入门
...
original_title: 懒猫微服实战入门（九）：给懒猫安装图形界面，使用 XRDP 实现远程桌面
---

需要远程打开 Linux 浏览器或图形工具时，可以用 XRDP 连接轻量级 XFCE 桌面。本文记录基于 Debian 的系统中安装桌面、配置会话和连接测试的步骤，实测设备是懒猫微服。它的重启还原机制需要另外处理，普通 Debian 或 Ubuntu 主机应按自身的服务和数据持久化方式配置。

本文属于「懒猫微服实战入门 · 第九篇」。[查看系列目录](/topics/#lazycat) · [用 systemd 用户服务运行环境初始化脚本](/95e86a3c/)。
