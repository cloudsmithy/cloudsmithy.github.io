...
description: 懒猫微服不只是 NAS，它构建了从虚拟化到应用层的完整私有云生态，适合个人和小团队的云计算需求。
tags:
  - NAS
  - 懒猫微服
toc: true
...
懒猫微服正是基于这一架构理念，构建了完整的私有云计算平台。下面按这三层分别看懒猫微服是怎么实现的。

### 懒猫微服的 IaaS 实现：虚拟化基础设施

#### IaaS 层的技术本质

先从最底层说起。IaaS 提供的是虚拟化的计算基础设施——虚拟机实例、块存储（Block Storage）、对象存储（Object Storage）、虚拟网络（VPC）、负载均衡器。基础设施的控制权全在你手上，代价是管理责任也一并归你：操作系统、中间件、应用程序的安装配置，系统更新、安全补丁、网络策略的维护。AWS EC2、阿里云 ECS、Azure Virtual Machines 走的都是这条路。
...
懒猫微服基于 Docker 容器引擎，构建了三层容器管理架构，逐层看：
