...
description: 懒猫微服 OIDC 单点登录开发教程（下篇），实战代码与调试。
tags:
  - SSO
  - 懒猫微服
toc: true
...
`/callback` 处理函数先从回调参数取出授权码，再用会话里的 code verifier 按 PKCE + 授权码模式向令牌端点换取 access token 和 ID token；成功后用 access token 调 `/userinfo` 获取用户资料。为了方便观察协议返回值，下面的调试代码会暂时把三者一起返回给浏览器。

> **仅供本地调试**
>
> 下面原样返回 `Access Token`、`ID Token`，并打印完整 `token_data`，只是为了在隔离的本地环境中观察 OIDC 流程。Token 属于敏感凭证，可能被浏览器历史、截图、日志、插件、代理或前端脚本读取。不要把这段写法部署到公网或生产环境；正式应用应把 Token 留在服务端，完成 ID Token 验签，只向前端返回必要的用户字段，或者使用服务端会话维持登录状态。
...
def callback():
    ...
    # 仅供本地调试：生产环境不要把 Access Token / ID Token 原样返回浏览器
    return {
...
这一步的目的只是确认换码结果。正式应用不应把原始 Token 当作页面数据返回；更稳妥的做法是服务端完成验签并保存必要状态，然后重定向到业务页面，只展示经过筛选的用户资料。
...
![OIDC 授权码流程：用户授权、Code 换取 Token，再访问受保护资源](https://raw.githubusercontent.com/cloudsmithy/picgo-imh/master/7a6f946c-b299-410b-8286-2958d488caa2.png)
...
3. **核心路由**
   ...
   - `/callback`：
     ...
     4. 为了本地调试，以 JSON 形式返回令牌与用户资料；生产环境不要保留这个返回方式。

> **仅供本地调试**：下面的完整代码仍会打印 `token_data`，并把 `Access Token`、`ID Token` 原样返回浏览器。生产环境应删除这些调试输出与返回字段，关闭 `debug=True`，使用 HTTPS，校验 `state` 参数防 CSRF，验证 ID Token 的签名、`iss`、`aud`、`exp` 和 `nonce`，并妥善处理 Token 异常和错误分支。
...
if __name__ == '__main__':
    app.run(debug=True)  # 仅限本地调试，生产环境必须关闭 debug
...
> **仅供本地调试**：下面为了查看协议结果，仍把 `access_token` 和 `id_token` 传给了 `callback.html`。模板只应在本机调试时使用，不能公开部署。生产页面只需要接收经过筛选的 `user_info`，原始 Token 应留在服务端。
...
def callback():
    ...
    # 仅供本地调试：生产环境不要把原始 Token 传给浏览器模板
...
if __name__ == "__main__":
    app.run(debug=True, port=5005)  # 仅限本地调试，生产环境必须关闭 debug
