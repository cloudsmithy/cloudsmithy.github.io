...
description: 借助懒猫微服穿透服务，让车机远程播放群晖 DS music，免洗白方案
tags:
  - NAS
  - 懒猫微服
toc: true
...
> 声明：炫技篇不是最佳实践，只是为了记录过程和那颗折腾的心。虽然很多时候花钱升级，或者多忍一忍就好了，但折腾的心始终不安分。技术在职场中证明不了什么，但是在生活中可以。当晦涩的理论不再只存在于书本，才能给我们带来更大的价值。这个过程也能让我找到自己为什么需要这项技术、问题的痛点，以及它的前世今生……

前些天给老旧的车载导航升级了 Wi-Fi 功能。大致过程是：出厂时锁住了 Wi-Fi 驱动和 UI，我通过 ADB 解锁，再通过路由器 Mesh 更新老旧 App 和导航数据。这次又准备增加一个新功能，给普通车机升级 CarPlay。
...
那我如果不用安卓，换 CarPlay 呢？是不是就能用 iOS 客户端直接投过去了？

### Part 2：

开始改 CarPlay。

车载导航是当时赠送的，系统是 Android 6.0。朋友推荐说可以买 CarPlay 盒子，于是我疯狂搜索。现在的 CarPlay 盒子大致分为两种：

1. 有线 CarPlay 改无线 CarPlay
2. 安卓车机改无线 CarPlay

我的安卓车机没有 CarPlay App，方向盘也没有语音控制按钮，所以一开始我很怀疑它能不能支持 CarPlay。直到搜到一篇夸赞车连易的帖子，大意是这家公司很早就在做这个方向，产品比较稳定。

买回来以后先安装 AutoKit 检测软件（购买前需要用 iPhone 检测），这个 App 后续也会作为 CarPlay 入口使用。这里还有个小插曲：之前用 ADB 调试时改过工厂模式并打开 USB 调试，导致后来 USB 无法识别任何设备，包括 U 盘、2.4 GHz 发射器、手机和 CarPlay 盒子。对照之前的照片把设置关掉，设备就恢复正常了。然后我拥有了人生第一台 CarPlay 车机，盒子一点不烫，就是手机哗哗掉电。

![image-20250519062925067](https://raw.githubusercontent.com/cloudsmithy/picgo-imh/master/image-20250519062925067.png)

开心之余还不忘去找懒猫 App。屏幕上有钉钉、百度网盘、网易云，甚至连 Home Assistant 都有，那我的懒猫微服 App 呢？原来是 Apple 的白名单限制，美其名曰为了安全。让 GPT 搜了一下，基本上除了导航和音乐类 App，其他都没有。那我在高速服务区追个剧？好歹让投个屏嘛……

同时和微服以及 CarPlay 盒子厂家确认，得到了一份寥寥无几的 App 名单。
...
> **版本提示（2026 年 9 月）**
>
> 下面这部分整理自文章写作时可见的 Apple 文档和 iOS 18.4 公开信息。CarPlay 支持的 App 类别、审核要求和地区限制可能继续变化，请以当前 Apple Developer 文档为准。

以下内容可作为当时 CarPlay 官方支持的 App 类型、代表性应用和常见限制速查表。([Apple Developer](https://developer.apple.com/carplay/documentation/CarPlay-App-Programming-Guide.pdf))
...
于是把群晖端口映射出来，就可以在 App 中远程连接使用了，这不比 Synology QuickConnect 还省事嘛……
...
用懒猫微服解决了公网转发问题，再也不用担心运营商封端口了。再加上车载大屏，除了有点掉帧，也算圆了一个梦吧。
