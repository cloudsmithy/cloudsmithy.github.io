...
前面我们聊了用 `systemctl --user` 在开机时自动安装软件，解决重启丢包的问题。但说实话，每次开机都要跑一遍 `apt install`，总觉得不够优雅。
...
这篇文章就来聊聊怎么用 Docker 容器跑系统监控工具。不用装软件，不怕重启丢失，一条命令就能用，用完自动清理。对于我们这种既想玩命令行、又不想把系统搞乱的人来说，简直完美。
 <!-- more -->
