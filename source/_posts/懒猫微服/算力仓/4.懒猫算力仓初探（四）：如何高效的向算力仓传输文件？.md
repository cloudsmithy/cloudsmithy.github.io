...
description: 介绍几种高效向懒猫算力仓传输文件的方法
tags:
  - NAS
  - 懒猫微服
toc: true
...
算力仓和微服不一样的一点是，它没跑网盘服务。用惯了微服那个网盘再回来，第一反应就是：那我代码怎么进去，跑完的模型权重怎么出来。

绕不过去的三个问题，我自己也是这么一个个撞上的：

- 本地写好的脚本，怎么同步进去跑？
- 模型权重几十个 G，是不是拿 U 盘拷更快？
- 推理结果怎么捞回客户端？

其实没什么特别的。核心虽然是 **NVIDIA Jetson AGX Orin** 开发板，但跑的就是个 Ubuntu 22.04，Linux 服务器那套传文件的办法照搬就行。

下面按 **命令行**、**图形化**、**高效同步**、**临时传输**、**云同步** 五类过一遍，各自适合什么场景我也标出来。
