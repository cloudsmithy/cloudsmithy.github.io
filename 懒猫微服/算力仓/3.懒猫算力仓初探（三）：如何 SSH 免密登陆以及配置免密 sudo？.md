---
title: 懒猫算力仓初探（三）：如何配置 SSH 免密登录和免密 sudo？
description: 配置算力仓 SSH 免密登录和免密 sudo 的详细教程
tags:
  - NAS
  - 懒猫微服
toc: true
...
算力仓拿到手先插了显示器，敲了两天就受不了了——机器搁在弱电箱边上，每次调试都得搬个屏幕过去。于是还是回到 SSH。

好消息是出厂就开了 SSH 服务端，什么都不用装。所以第一件事只是把它的 IP 找出来。

### 一、获取 IP 地址

如果你使用路由器，可以在路由器后台查看分配到的 IP，并用 `telnet <IP 地址> 22` 或 `nc -zv <IP 地址> 22` 测试 SSH 端口是否开放：
...
进入算力仓后，使用 `visudo` 编辑配置。它会在保存前检查语法，避免错误配置导致 sudo 无法使用：
...
sudo visudo -cf /etc/sudoers.d/nvidia
```
...
这样配置下来，你就能在懒猫算力仓上实现“一步登录 → 直接 sudo”的丝滑体验。无论是本地调试模型，还是在局域网中远程部署服务，都可以通过一条命令进入需要的管理环境。
