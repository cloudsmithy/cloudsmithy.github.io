---
title: systemd 用户服务：用 systemctl --user 在登录时运行脚本
description: 为初始化脚本编写 systemd 用户服务，配置 oneshot、default.target 和日志排查，并记录懒猫微服恢复工具环境的实践。
tags:
- Linux
- 懒猫微服
toc: true
...
original_title: 懒猫微服进阶心得（四）：每次重启都丢配置？用 systemctl --user 自动装回环境！
---

只为某个用户运行初始化脚本时，可以把服务放进 ~/.config/systemd/user/，再用 systemctl --user 管理。本文以一次性任务为例，记录服务文件、启用和状态检查。用户登录时启动与整机开机时启动需要区分；下面保留懒猫微服重启还原、主目录持久化这一具体背景。

本文属于「懒猫微服进阶心得 · 第四篇」。[查看系列目录](/topics/#lazycat) · [把用户服务用于远程桌面环境的恢复](/8e360f7e/)。
