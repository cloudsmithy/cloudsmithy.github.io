---
title: Docker 跨架构运行：ARM 与 x86 镜像选择、QEMU 和迁移
description: 在 ARM 与 x86 主机之间选择、验证和运行 Docker 镜像，排查 exec format error，并使用 save、load 迁移镜像。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 懒猫微服进阶心得（八）：如何在 ARM 机器上下载并运行X86镜像以及在X86上运行ARM镜像
---

在 Apple Silicon、树莓派和 x86 服务器之间搬运容器时，镜像的平台信息和宿主机架构必须分清。本文从 --platform 参数讲起，记录架构验证、QEMU 模拟以及镜像保存和加载的方法。下面的懒猫微服案例，是我把这些命令用于跨设备部署的过程。

本文属于「懒猫微服进阶心得 · 第八篇」。[查看系列目录](/series/docker/) · [先理解镜像分层、仓库与导入导出](/e0565a55/)。
...
下面说说怎么在 ARM 设备上拉起 X86 镜像，以及镜像的保存和加载。
