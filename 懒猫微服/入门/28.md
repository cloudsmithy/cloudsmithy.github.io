...
description: 懒猫微服的 heiyu.space 域名背后是 IPv6，配合 iperf3 和 LibreSpeed 实测 1G 与 2.5G 环境下的有线无线速率。
tags:
  - 懒猫微服
toc: true
series: 懒猫微服入门
...
玩了这么多年 NAS，习惯都是记 IP 地址。懒猫微服这边不给你裸 IP，直接发一个 **`<机器名>.heiyu.space`** 的域名，内网外网都用同一个地址访问。

我平时在公网和私网之间来回切，一开始还挺不适应，就想看看它背后到底是怎么转的、速度又能跑到多少。

先 `dig AAAA <机器名>.heiyu.space` 解一下，背后是个 IPv6 地址。具体的转发机制我研究了挺久也没弄明白，只能给个结论：内网互联网都能通。要提醒一句的是，如果你内网挂了代理，流量很可能被代理拽出去绕一圈再回来，规则得手动改。
