---
title: Docker 镜像（二）：分层、构建、仓库与镜像优化
description: 理解 Docker 镜像的分层结构，练习构建、拉取、推送、导出与导入，并用多阶段构建和缓存优化镜像。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 写给懒猫微服玩家的容器小书 Docker篇（二）：《镜像旅馆的秘密》
---

镜像为什么会越做越大，改了一行代码为什么要重新构建，离线机器又该如何导入镜像？这一章围绕镜像的分层、版本、仓库和构建缓存展开，示例可以用于普通 Linux 主机、Docker Desktop，也可以继续用于 NAS 上的容器部署。

本文属于「容器小书 · 第二章《镜像旅馆的秘密》」。[查看系列目录](/series/docker/) · [把镜像在 ARM 与 x86 设备之间迁移](/86ab1d7c/)。
...
> “镜像（Image）其实是一个**只读的分层文件系统**。Dockerfile 中修改文件系统的指令会产生镜像层；CMD 等配置指令主要写入镜像元数据。”
