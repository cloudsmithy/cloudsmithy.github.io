---
title: Docker Compose（五）：多容器编排、网络与多环境配置
description: 用 Docker Compose 管理应用、数据库与前端服务，配置网络、卷、环境变量和健康检查，并整理多环境覆盖与 V2 迁移。
tags:
- Docker
- 懒猫微服
toc: true
...
original_title: 写给懒猫微服玩家的容器小书Docker篇（五）：《多容器交响曲：Docker Compose 上场》
---

应用、数据库和前端各写一条 docker run，很快就会遇到配置分散、依赖顺序和重复部署的问题。本章用一份 Compose 文件组织多个服务，继续处理共享网络、数据卷、环境变量、就绪检查和多环境配置。示例从标准 Compose 写法出发，再连接到 Homelab 的部署场景。

本文属于「容器小书 · 第五章《多容器交响曲》」。[查看系列目录](/series/docker/) · [懒猫微服部署与容器实践](/topics/#lazycat)。
